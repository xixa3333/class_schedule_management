      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final double dayWidth =
              (constraints.maxWidth - _timeColumnWidth) / _totalDays;
                    ...
                    child: Stack(
                      children: [
                        ...
                        ..._courses.map((course) {
                          ...
                          if (course.startPeriod > settings.maxPeriods)
                            return const SizedBox();

                          // 座標計算保持不變
                          final double left =
                              _timeColumnWidth +
                              (course.dayOfWeek - 1) * dayWidth;
                          final double top =
                              (course.startPeriod - 1) * _periodHeight;

                          // [修正] 如果課程結束節次超過設定值，將高度截斷，避免超出邊界
                          int effectiveEnd = course.endPeriod;
                          if (effectiveEnd > settings.maxPeriods)
                            effectiveEnd = settings.maxPeriods;

                          final double height =
                              (effectiveEnd - course.startPeriod + 1) *
                              _periodHeight;
