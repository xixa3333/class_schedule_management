...
class _CourseEditPageState extends State<CourseEditPage> {
  ...
  bool _enableNotification = true; // 移入此處作為狀態變數
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (newCourse.startPeriod > newCourse.endPeriod) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('錯誤：開始節次不能晚於結束節次')));
        return;
      }
      ...
      if (_enableNotification) {
        try {
          ...
          String startTimeStr = settings.getPeriodStartTime(
            newCourse.startPeriod,
          );

          DateTime nextClassTime = _calculateNextClassTime(
            newCourse.dayOfWeek,
            startTimeStr,
          );
          ...
          // 【修正】使用一致的 ID 計算方式
          // 確保創建和刪除時使用相同的邏輯
          final int alarmId = newCourse.id.hashCode.abs() % 100000;

          await NotificationService.scheduleClassReminder(
            notificationId: alarmId,
            courseName: newCourse.courseName,
            classroom: newCourse.classroom?.isNotEmpty == true
                ? newCourse.classroom!
                : '未指定教室',
            classStartTime: nextClassTime,
          );
        } catch (e) {
          debugPrint("推播設定失敗: $e");
        }
  ...
  void _delete() async {
    ...
    if (confirm == true) {
      ...
      await NotificationService.cancelNotification(
        widget.course!.id.hashCode.abs() % 100000,
      );
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // 在 _noteCtrl 的 TextFormField 上方加入
            SwitchListTile(
              title: const Text('開啟課程提醒'),
              subtitle: const Text('於上課前 10 分鐘推播通知'),
  ...
  // [新增] 計算這堂課「下一次」上課的具體日期與時間
  DateTime _calculateNextClassTime(int dayOfWeek, String timeStr) {
    ...
    if (dayOfWeek > now.weekday) {
      // 例如今天是週一(1)，課是週三(3)，差2天
      final daysUntilNext = dayOfWeek - now.weekday;
      scheduledTime = now.add(Duration(days: daysUntilNext));
    } else if (dayOfWeek < now.weekday) {
      // 例如今天是週五(5)，課是週二(2)，下週才有
      final daysUntilNext = 7 - now.weekday + dayOfWeek;
      scheduledTime = now.add(Duration(days: daysUntilNext));
    } else {
      // 今天就是課程的日子，先設為今天
      scheduledTime = now;
